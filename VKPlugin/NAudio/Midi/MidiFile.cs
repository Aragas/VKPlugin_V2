            ...
            using (br)
            {
                ...
                for (int track = 0; track < tracks; track++)
                {
                                ...
                                if (strictChecking)
                                {
                                    if (br.BaseStream.Position < startPos + chunkSize)
                                    {
                                        throw new FormatException(
                                            String.Format("End Track event was not the last MIDI event on track {0}",
                                                track));
                                    }
                                }
                            }
                    ...
                    if (outstandingNoteOns.Count > 0)
                    {
                        if (strictChecking)
                        {
                            throw new FormatException(String.Format("Note ons without note offs {0} (file format {1})",
                                outstandingNoteOns.Count, fileFormat));
                        }
                    }
                    if (br.BaseStream.Position != startPos + chunkSize)
                    {
                        throw new FormatException(String.Format("Read too far {0}+{1}!={2}", chunkSize, startPos,
        ...
        public static void Export(string filename, MidiEventCollection events)
        {
            ...
            using (var writer = new BinaryWriter(File.Create(filename)))
            {
                ...
                for (int track = 0; track < events.Tracks; track++)
                {
                    ...
                    {
                        Debug.Assert(MidiEvent.IsEndTrack(eventList[eventList.Count - 1]),
